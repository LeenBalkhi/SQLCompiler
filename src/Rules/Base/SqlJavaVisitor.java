...
//                            tableSymbol.type = tableSymbol.name;
                            ...
                            symbolTable.tables.put(tableSymbol.name,tableSymbol);
                        }
                    }
                }
                else{
    ...
    @Override
    public Table visitTableAsValue(SqlParser.TableAsValueContext ctx)
    {
      Table table = new Table();
      table.TableName = visitAny_name(ctx.table_name().any_name()).name;
     return table;
    }
    @Override
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public SqlExpression visitExpr(SqlParser.ExprContext ctx)
    {
        ...
        Node temp = visit(ctx);
        if(temp instanceof SqlExpressionCase0){
            sqlExpression.expression = (SqlExpressionCase0)temp;
            sqlExpression.parseObject = ((SqlExpressionCase0)temp).parseObject;
        }
    ...
    @Override
    public SqlExpressionCase0 visitCase0(SqlParser.Case0Context ctx) {
        SqlExpressionCase0 sqlExpressionCase0 = new SqlExpressionCase0();
        sqlExpressionCase0.parseObject = symbolTable.queryManager.clone();
        return sqlExpressionCase0;
    }
    ...
    public SqlExpressionCase2 visitCase2(SqlParser.Case2Context ctx) {
        ...
        if(ctx.table!=null){
            sqlExpressionCase2.tableName = visitAny_name(ctx.table);
        }
        ...
        err = false;
//        if(ctx.any!=null){
//            sqlExpressionCase2.anyName = visitAny_name(ctx.any);
    ...
    public CreateTableStatement visitCreate_table_stmt(SqlParser.Create_table_stmtContext ctx)
    {
        ...
        if(ctx.table_name() != null)
        {
            ...
            if(symbolTable.sqlTypes.containsKey(tableSymbol.name)) {
                ...
                if(!err){
                    SqlType sqlType = tableSymbol.getSqlTypeFromTableSymbol(symbolTable);
                    createTableStatement.sqlType = sqlType;
                    tableSymbol.type = sqlType.name;
                    symbolTable.scopeStack.peek().symbolMap.put(tableSymbol.name , tableSymbol);
                    symbolTable.tables.put(tableSymbol.name,tableSymbol);
                    if(ctx.path()!=null){
                        String path = visitPath(ctx.path()).path;
                        createTableStatement.path = path;
...
//                        }
        ...
        {
            if(ctx.file().K_JSON() != null)
                createTableStatement.type = "json";
            if(ctx.file().K_CSV() != null)
                createTableStatement.type = "csv";
        }
        if(ctx.path()!=null){
            visitPath(ctx.path());
        }
...
//            }
            ...
            for (int i = 0; i < ctx.result_column().size(); i++) {
                if (visitResult_column(ctx.result_column().get(i)) != null) {
                    ...
                    if (expr.parseObject == null) {
                        ...
                        if (expr.parseObject instanceof ColumnSymbol) {
                            ...
                            tableSymbol.values.put(col.name, col);
                        } else if (expr.parseObject instanceof TableSymbol) {
                            TableSymbol temp = ((TableSymbol) expr.parseObject);
    ...
    public TableOrSubquery visitTable_or_subquery(SqlParser.Table_or_subqueryContext ctx)
    {
        ...
        if(ctx.table_name()!= null){
            ...
            if(ctx.table_alias()!=null)
                ...
                tableOrSubquery.type.name = RandomNameGenerator.generateNewRandomName();
                SqlType sqlType = tableOrSubquery.type.getSqlTypeFromTableSymbol(symbolTable);
                tableOrSubquery.type.type = sqlType.name;
                symbolTable.sqlTypes.put(sqlType.name,sqlType);
            }
        }
...
//            }
    ...
    public Path visitPath(SqlParser.PathContext ctx)
    {
        ...
        path.append(visitAny_name(ctx.any_name().get(0)).name);
        for(int i=1 ; i < ctx.any_name().size(); i++)
        {
            path.append("\\");
            path.append("\\");
            path.append(visitAny_name(ctx.any_name().get(i)).name);
        }
        path.append(".");
        if(ctx.file().K_CSV() != null)
            path.append("csv");
    ...
    public Path visitJar_path(SqlParser.Jar_pathContext ctx)
    {
        ...
        for(int i=0 ; i < ctx.any_name().size(); i++)
        {
            ...
            if(i< ctx.any_name().size()-1)
                ...
                path.setFilename(visitAny_name(ctx.any_name().get(i)).name+=".jar");
        }
        return path;
    }
...
//    }
    ...
    public AggregationFunction visitCreate_agg_fun(SqlParser.Create_agg_funContext ctx)
    {
        ...
        aggregationFunction.setAggregationFunctionName(visitAny_name(ctx.function_name().any_name()).name);
        aggregationFunction.setClassName(visitAny_name(ctx.class_name().any_name().any_name()).name);
        aggregationFunction.setMethodName(visitAny_name(ctx.method_name().any_name().any_name()).name);
